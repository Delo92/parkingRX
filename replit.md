...
- **Manual Payment**: Admin can process manual payments for applicants from the Users page (green $ button). Opens a dialog to select a package and enter a reason. Creates the application with paid status and triggers the full pipeline (auto-complete or doctor review). Endpoint: `POST /api/admin/users/:userId/manual-payment`.
...
- **Package-Specific Required Fields**: Admins define custom fields per package (text, textarea, email, phone, number, date, dropdown, radio buttons). Fields are rendered dynamically in the applicant wizard. Required fields are validated before proceeding. Field values stored in `formData` and passed through to PDF auto-fill via `patientData`. Radio auto-fill maps: `placardType` (New/Renewal/Replacement → radio 1-3), `placardCount` (1/2 → radio 4-5), `disabilityCondition` (A-H → radio 7-14), `duration` (Temporary/5-year → radio 15-16).
