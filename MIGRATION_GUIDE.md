...
## CHANGE 13: Document Auto-Generation Pipeline with Template Rendering

### WHY
The original `autoGenerateDocument` function only stored basic metadata when a doctor approved an application. It didn't use the doctor's HTML form template or substitute placeholder values. The new version renders the doctor's custom `formTemplate` (or a default template) with all 26 placeholder tags replaced by real patient/doctor/application data, producing a complete HTML document that patients can view and print from their dashboard.

### HOW IT WORKS
1. **Template Rendering** (`renderTemplate` helper in `server/routes.ts`): Takes an HTML template string and a `Record<string, string>` of placeholder replacements. Uses regex to replace all `{{placeholderName}}` occurrences.
2. **`autoGenerateDocument` function** (upgraded): Collects all data — patient profile, doctor profile, package info, form data — into a 26-key `placeholders` object. If the doctor has a custom `formTemplate`, it renders that. Otherwise, it generates a professional default HTML document with patient info, service details, and certifying physician credentials.
3. **Generated HTML stored in metadata**: The `generatedHtml` field is saved inside the document's `metadata` JSON blob in Firestore.
4. **New API endpoints**:
   - `GET /api/documents` — Returns all documents for the authenticated user
   - `GET /api/documents/:id` — Returns a single document (user must own it or be Level 3+)
   - `GET /api/documents/:id/html` — Serves the generated HTML as a full printable page with Print/PDF button. Supports `?token=` query param for new-tab authentication.
5. **DocumentsPage upgraded** (`client/src/pages/dashboard/applicant/DocumentsPage.tsx`): Now fetches from `/api/documents`, displays completed certificates with View and Print/PDF buttons. Opens document HTML in a new tab using Firebase ID token for authentication.
...
| `{{patientDOB}}` | `patient.dateOfBirth` or `formData.dateOfBirth` |
| `{{patientPhone}}` | `patient.phone` or `formData.phone` |
| `{{patientEmail}}` | `patient.email` |
| `{{patientAddress}}` | `patient.address` |
| `{{patientCity}}` | `patient.city` |
| `{{patientState}}` | `patient.state` |
| `{{patientZipCode}}` | `patient.zipCode` |
| `{{patientSSN}}` | `formData.ssn` or `patient.ssn` |
| `{{patientDriverLicense}}` | `formData.driverLicenseNumber` |
| `{{patientMedicalCondition}}` | `formData.medicalCondition` |
| `{{reason}}` | `formData.reason` |
| `{{packageName}}` | `package.name` |
| `{{date}}` | Current date (long format) |
| `{{dateShort}}` | Current date (MM/DD/YYYY) |

### FILES CHANGED
- `server/routes.ts` — `renderTemplate()` helper, upgraded `autoGenerateDocument()`, 3 new `/api/documents` endpoints
- `client/src/pages/dashboard/applicant/DocumentsPage.tsx` — Full rewrite: fetches real documents, displays certificates, View/Print buttons
